            ...
            exec {
                commandLine(
                    "sh", "-c", """
                        ...
                        echo "pwd stampato" &&
                        docker run -v \"${project.projectDir}"/../:/workspace -v /workspace/tmp/.gradle -v \"${project.projectDir}"/../.gradle:/tmp/.gradle --name ubuntu-opencv_build-container --rm ubuntu-opencv_build /bin/bash -c 'GRADLE_USER_HOME=/tmp/.gradle ./gradlew build'
                    """)
                standardOutput = System.out
                errorOutput = System.err
            }
