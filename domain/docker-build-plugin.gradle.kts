    ...
    if (OperatingSystem.current().isWindows) {
        ...
        doLast {
            exec {
                commandLine(
                    "sh", "-c", """
                        docker run -v "${project.projectDir}"/../:/workspace:ro -v "${project.projectDir}/.gradle":/workspace/.gradle --name ubuntu-opencv_build-container --rm ubuntu-opencv_build /bin/bash -c 'cd /workspace && GRADLE_USER_HOME=/workspace/.gradle ./gradlew build'
                    """)
                standardOutput = System.out
                errorOutput = System.err
            }
